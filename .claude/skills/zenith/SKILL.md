...
# /zenith — Run a mission with the Zenith orchestrator

## What this skill does

When you run `/zenith <prompt>`, this skill tells the host agent to switch into **Zenith Orchestrator** mode for the duration of the mission. Instead of doing the work directly, the host will:

1. Read `.claude/orchestrator_prompt.md` and treat its contents as its primary operating role.
...
The host agent will read `.claude/orchestrator_prompt.md`, assume the orchestrator role, and run Zenith against your prompt.

## First line of every mission

The canonical first message Zenith expects is:
...
(`/zenith` packages that exact prompt for you — you only need to write the instruction.)

## Is Zenith running? Start it if not

Zenith is **lazy**: the CLI installs on demand, but a project's MCP wiring only happens after `zenith init` has staged `.mcp.json` (and `.codex/config.toml` for Codex) into your workspace, and the host agent has registered the MCP server. Check before you start a mission.
...
# Hermes ships its own ACP runtime — no extra install needed.
...
- `.mcp.json` (Claude / Hermes) or `.codex/config.toml` (Codex) — points the host agent at the `zenith-server` MCP server.
- `.claude/agents/`, `.codex/agents/`, or `.hermes/agents/` — subagent definitions (e.g. `contract-review`, `feature-reviewer`, `flow-validator`, `investigator`).
- `.claude/skills/`, `.codex/skills/`, `.hermes/skills/` — bundled Zenith skills (`engineering-mission-playbook`, `optimization-mission-playbook`, `agent-browser`, `scrutiny-validator`, `user-testing-validator`, `benchmark-validator`).
- `.claude/orchestrator_prompt.md` (or `.codex/` / `.hermes/` equivalent) — the system prompt that turns the host agent into the Zenith Orchestrator.

### Per-session startup

Launch the host agent **from inside the initialized workspace** so the MCP wiring is picked up:
