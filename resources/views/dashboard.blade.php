...
<head>
    ...
    <style>
        ...
        .donut-swing { fill: none; stroke-width: 28; stroke-linecap: round; transition: stroke-dashoffset 1.2s cubic-bezier(0.4, 0, 0.2, 1); }
        ...
        .bar-stack { display: flex; gap: 2px; align-items: flex-end; height: 96px; width: 100%; justify-content: center; }
        .bar-segment { width: 18px; border-radius: 4px 4px 0 0; transition: height 0.8s cubic-bezier(0.4, 0, 0.2, 1); cursor: pointer; min-height: 4px; }
        .bar-segment.in { background: #3b82f6; }
        .bar-segment.out { background: #f87171; }
        .bar-label { font-size: 9px; color: #94a3b8; font-weight: 600; letter-spacing: 0.02em; }
        ...
        /* ── Grouped table ───────────────────────────────────────────── */
        .day-row { @apply flex items-center justify-between px-4 py-2.5 rounded-2xl cursor-pointer transition-all; }
        .day-row:hover { @apply bg-gray-50; }
        .day-row.open { @apply bg-blue-50 border border-blue-100; }
        .day-items { overflow: hidden; max-height: 0; transition: max-height 0.3s ease; }
        .day-items.open { max-height: 600px; }
        .tx-row { @apply flex items-center justify-between px-4 py-2 rounded-xl transition; }
        ...
        @keyframes pulse-ring { 0% { box-shadow: 0 0 0 0 rgba(59, 130, 246, 0.3); } 70% { box-shadow: 0 0 0 8px rgba(59, 130, 246, 0); } 100% { box-shadow: 0 0 0 0 rgba(59, 130, 246, 0); } }
        ...
        /* ── Modal overlay ─────────────────────────────────────────── */
        .modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.5); backdrop-filter: blur(4px); z-index: 100; display: flex; align-items: center; justify-content: center; opacity: 0; pointer-events: none; transition: opacity 0.3s ease; }
        .modal-overlay.active { opacity: 1; pointer-events: auto; }
        .modal-box { background: white; border-radius: 2rem; padding: 2rem; max-width: 480px; width: 90%; transform: scale(0.9) translateY(20px); transition: transform 0.4s cubic-bezier(0.34,1.56,0.64,1); position: relative; }
        .modal-overlay.active .modal-box { transform: scale(1) translateY(0); }

        /* ── Split slider ──────────────────────────────────────────── */
        .split-track { position: relative; height: 14px; background: linear-gradient(to right, #22c55e 0%, #22c55e 50%, #3b82f6 50%, #3b82f6 100%); border-radius: 100px; margin: 1rem 0; }
        .slider-thumb { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 28px; height: 28px; background: white; border: 3px solid #fbbf24; border-radius: 50%; cursor: grab; box-shadow: 0 2px 8px rgba(0,0,0,0.2); display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 900; color: #d97706; transition: box-shadow 0.2s, left 0.1s; z-index: 2; }
        .slider-thumb:active { cursor: grabbing; box-shadow: 0 4px 16px rgba(251,191,36,0.5); }

        /* ── Split / category cards ────────────────────────────────── */
        .choice-card { border-radius: 1.5rem; padding: 1rem; text-align: center; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); border: 2px solid transparent; cursor: pointer; }
        ...
        .choice-card.active { transform: scale(1.05); box-shadow: 0 4px 20px rgba(0,0,0,0.1); }
        .choice-card:hover { filter: brightness(0.95); }

        /* ── Preset pills ──────────────────────────────────────────── */
        .preset-pill { padding: 6px 14px; border-radius: 100px; font-size: 12px; font-weight: 700; border: 2px solid; transition: all 0.2s; cursor: pointer; background: white; }
        ...
        /* ── Confetti ─────────────────────────────────────────────── */
        @keyframes confetti-fall { 0% { transform: translateY(0) rotate(0deg) translateX(0); opacity: 1; } 100% { transform: translateY(400px) rotate(720deg) translateX(var(--tx, 50px)); opacity: 0; } }
        .confetti-particle { position: fixed; width: 8px; height: 8px; border-radius: 2px; animation: confetti-fall 2s ease-out forwards; z-index: 9999; pointer-events: none; }

        /* ── Coin flip ────────────────────────────────────────────── */
        @keyframes coin-flip { 0% { transform: rotateY(0deg) scale(1); } 30% { transform: rotateY(180deg) scale(1.1); } 50% { transform: rotateY(360deg) scale(1.1); } 70% { transform: rotateY(540deg) scale(1.05) translateY(-8px); } 100% { transform: rotateY(720deg) scale(1) translateY(0); } }
        .coin-flip { animation: coin-flip 0.8s ease-in-out forwards; }

        /* ── Toast ────────────────────────────────────────────────── */
        @keyframes toast-in { 0% { transform: translateX(120%); opacity: 0; } 10% { transform: translateX(0); opacity: 1; } 85% { transform: translateX(0); opacity: 1; } 100% { transform: translateX(120%); opacity: 0; } }
        .toast { position: fixed; top: 80px; right: 20px; z-index: 200; background: white; border-radius: 1rem; padding: 0.875rem 1.25rem; box-shadow: 0 10px 40px rgba(0,0,0,0.15); animation: toast-in 4s ease-in-out forwards; border-left: 4px solid #22c55e; max-width: 320px; display: flex; align-items: center; gap: 0.5rem; }
        .toast.wan { border-left-color: #f472b6; }
        .toast.info { border-left-color: #3b82f6; }

        /* ── Badge bounce ─────────────────────────────────────────── */
        @keyframes badge-bounce { 0% { transform: scale(0); opacity: 0; } 60% { transform: scale(1.15); opacity: 1; } 100% { transform: scale(1); opacity: 1; } }
        .badge-new { animation: badge-bounce 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }

        /* ── Fire flicker ─────────────────────────────────────────── */
        @keyframes fire-flicker { 0%, 100% { transform: scale(1) rotate(-3deg); } 50% { transform: scale(1.15) rotate(3deg); } }
        .fire-icon { display: inline-block; animation: fire-flicker 0.4s ease-in-out infinite; }

        /* ── Golden glow for high savings ─────────────────────────── */
        @keyframes golden-pulse { 0%, 100% { box-shadow: 0 0 0 0 rgba(251,191,36,0); } 50% { box-shadow: 0 0 20px 4px rgba(251,191,36,0.4); } }
        .high-savings { animation: golden-pulse 2s ease-in-out infinite; }

        /* ── Category chips ───────────────────────────────────────── */
        .cat-chip { display: inline-flex; align-items: center; gap: 4px; padding: 4px 12px; border-radius: 100px; font-size: 12px; font-weight: 700; cursor: pointer; transition: all 0.2s; border: 2px solid transparent; }
        ...
        /* ── Need/Want toggle ─────────────────────────────────────── */
        .nw-toggle { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
        .nw-btn { padding: 1rem; border-radius: 1.5rem; border: 3px solid transparent; cursor: pointer; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); text-align: center; }
        ...
        .split-bar { height: 5px; border-radius: 3px; background: linear-gradient(to right, #22c55e var(--save-pct), #3b82f6 var(--save-pct)); margin-top: 6px; max-width: 200px; }

        /* ── CTA banners ───────────────────────────────────────────── */
        .cta-banner { border-radius: 1.5rem; padding: 1rem 1.25rem; display: flex; align-items: center; justify-content: space-between; gap: 1rem; border: 1px solid; }
        ...
        .cat-reveal { overflow: hidden; max-height: 0; opacity: 0; transition: max-height 0.4s ease, opacity 0.3s ease, margin 0.3s ease; }
...
<body class="bg-gray-50 min-h-screen">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
            ...
            <div class="flex flex-col md:flex-row items-center gap-6">
                ...
                <div class="text-center md:text-left flex-1">
                    <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">
                        <i class="ph ph-wallet mr-1"></i>Net Balance
                        @php
                            $filterLabels = ['today' => 'Today', 'week' => 'This Week', 'month' => 'This Month', 'year' => 'This Year', 'all' => 'All Time'];
                            $filterLabel = $filterLabels[$filter] ?? 'This Month';
                        @endphp
                        <span class="ml-2 text-blue-500 font-semibold normal-case tracking-normal">({{ $filterLabel }})</span>
                    </p>
                    <p class="text-4xl md:text-5xl font-black {{ $netBalance >= 0 ? 'text-green-600' : 'text-red-500' }} count-anim">
                        @if($netBalance >= 0)<span class="inline-block float">+</span>@endif
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3 flex-1">
                    <div class="bg-blue-50 rounded-2xl p-3 text-center">
                        <p class="text-[10px] font-bold text-blue-400 uppercase tracking-wider mb-1">Income</p>
                        <p class="text-lg font-black text-blue-600">Rp {{ number_format($totalIn, 0) }}</p>
                    </div>
                    <div class="bg-red-50 rounded-2xl p-3 text-center">
                        <p class="text-[10px] font-bold text-red-400 uppercase tracking-wider mb-1">Expenses</p>
                        <p class="text-lg font-black text-red-500">Rp {{ number_format($totalOut, 0) }}</p>
                    </div>
                    <div class="bg-emerald-50 rounded-2xl p-3 text-center">
                        <p class="text-[10px] font-bold text-emerald-400 uppercase tracking-wider mb-1">Saved</p>
                        <p class="text-lg font-black text-emerald-600">Rp {{ number_format($totalSaved, 0) }}</p>
                    </div>
                    <div class="bg-gray-50 rounded-2xl p-3 text-center">
                        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Txns</p>
                        <p class="text-lg font-black text-gray-600">{{ $transactionCount }}</p>
                    </div>
                </div>

                <!-- Health Score (compact inline) -->
                <div class="text-center shrink-0" title="Health Score = (Total Saved ÷ Total Income) × 100&#10;&#10;Saved: Rp {{ number_format($totalSaved, 0) }}&#10;Income: Rp {{ number_format($totalIn, 0) }}&#10;&#10;Result: ({{ number_format($totalSaved, 0) }} ÷ {{ number_format($totalIn > 0 ? $totalIn : 1, 0) }}) × 100 = {{ $healthScore }}">
                    @php
                        $hs = $healthScore;
                        $hsColor = $hs >= 70 ? '#22c55e' : ($hs >= 40 ? '#f59e0b' : '#ef4444');
                        $hsLabel = $hs >= 70 ? 'Healthy' : ($hs >= 40 ? 'Building' : 'Needs Focus');
                        $hsBg = $hs >= 70 ? 'bg-green-100 text-green-700' : ($hs >= 40 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-600');
                        $hsIcon = $hs >= 70 ? '💚' : ($hs >= 40 ? '🟡' : '🔴');
                        $hsCirc = 2 * 3.14159 * 32;
                    @endphp
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">
                        <i class="ph ph-heartbeat mr-1"></i>Health Score
                    </p>
                    <div class="relative w-20 h-20">
                        <svg width="80" height="80" viewBox="0 0 80 80">
                            ...
                            <circle class="gauge-fill" cx="40" cy="40" r="32"
                                stroke="{{ $hsColor }}"
                                stroke-dasharray="{{ $hsCirc }}"
                                stroke-dashoffset="{{ $hsCirc * (1 - $hs/100) }}"
                                transform="rotate(-90 40 40)"
                                id="healthGauge"/>
                        </svg>
                        <div class="absolute inset-0 flex flex-col items-center justify-center">
                            <span class="text-2xl font-black text-gray-900">{{ $hs }}</span>
                            <span class="text-[9px] font-bold text-gray-400">/100</span>
                        </div>
                    </div>
                    <span class="health-pill {{ $hsBg }} mt-1 inline-flex">{{ $hsIcon }} {{ $hsLabel }}</span>
                </div>
            </div>
        </div>

        <!-- ═══════════════════════════════════════════════════════════════
             GROUP 2: INSIGHTS ROW
        ═══════════════════════════════════════════════════════════════ -->
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">

            <!-- Save vs Spend (3/5 width) -->
            <div class="card lg:col-span-3">
                <h3 class="text-sm font-bold text-gray-700 mb-1">
                    <i class="ph ph-scales text-emerald-500 mr-1"></i>Save vs Spend
                </h3>
                <p class="text-xs text-gray-400 mb-4">Income allocation</p>

                <div class="flex flex-col sm:flex-row items-center gap-4 mb-4">
                    <!-- Donut -->
                    @php
                        $svCirc = 2 * 3.14159 * 50;
                        $svSaveDash = $svCirc * ($splitSavingsRate / 100);
                        $svSpendDash = $svCirc * ((100 - $splitSavingsRate) / 100);
                    @endphp
                    <div class="donut-wrap shrink-0" style="width:130px;height:130px">
                        <svg width="130" height="130" viewBox="0 0 130 130">
                            <circle class="donut-track" cx="65" cy="65" r="50" stroke-width="20"/>
                            @if($splitCount > 0)
                            <circle class="donut-swing" cx="65" cy="65" r="50" stroke="#22c55e"
                                stroke-dasharray="{{ $svSaveDash }} {{ $svCirc }}"
                                stroke-dashoffset="0" transform="rotate(-90 65 65)"/>
                            <circle class="donut-swing" cx="65" cy="65" r="50" stroke="#3b82f6"
                                stroke-dasharray="{{ $svSpendDash }} {{ $svCirc }}"
                                stroke-dashoffset="{{ -$svSaveDash }}" transform="rotate(-90 65 65)"/>
                            @endif
                        </svg>
                    ...
                    <div class="flex-1 grid grid-cols-2 gap-3">
                        <div class="bg-emerald-50 rounded-2xl p-3 text-center {{ $splitSavingsRate >= 60 && $splitCount > 0 ? 'high-savings' : '' }}">
                            <p class="text-[10px] font-bold text-emerald-500 uppercase tracking-wider mb-0.5">💰 Saved</p>
                            <p class="text-lg font-black text-emerald-700">Rp {{ number_format($totalSaved, 0) }}</p>
                            <p class="text-[10px] text-emerald-400">{{ $splitCount }} split{{ $splitCount != 1 ? 's' : '' }}</p>
                        </div>
                        <div class="bg-blue-50 rounded-2xl p-3 text-center">
                            <p class="text-[10px] font-bold text-blue-500 uppercase tracking-wider mb-0.5">🛒 Allocated</p>
                            <p class="text-lg font-black text-blue-700">Rp {{ number_format($totalSpent, 0) }}</p>
                            <p class="text-[10px] text-blue-400">for spending</p>
                        </div>
                    </div>
                </div>

                <!-- Streak + Badges -->
                @if($streak > 0 || count($badges) > 0)
                <div class="flex flex-wrap items-center gap-3 mb-3">
                    @if($streak > 0)
                    <div class="bg-amber-50 rounded-xl py-1.5 px-3">
                ...
                <p class="text-[10px] italic text-gray-400 leading-tight">{{ $saveVsSpendMessage }}</p>

                <!-- Need vs Want (compact bar inside Save vs Spend card) -->
                ...
                <div class="mt-4 pt-3 border-t border-gray-100">
                    <div class="flex items-center justify-between mb-1.5">
                        ...
                        <p class="text-[10px] text-gray-400 italic">{{ $needVsWantMessage }}</p>
                    </div>
                    <div class="flex gap-1 h-5 rounded-full overflow-hidden">
                        <div class="bg-emerald-400 transition-all duration-500 flex items-center justify-center"
                            style="width: {{ $needPct }}%"
                            title="Need: {{ round($needPct) }}%">
                            @if($needPct >= 15)
                            <span class="text-[9px] font-black text-white whitespace-nowrap">{{ round($needPct) }}%</span>
                            @endif
                        </div>
                        <div class="bg-pink-400 transition-all duration-500 flex items-center justify-center"
                            style="width: {{ $wantPct }}%"
                            title="Want: {{ round($wantPct) }}%">
                            @if($wantPct >= 15)
                            <span class="text-[9px] font-black text-white whitespace-nowrap">{{ round($wantPct) }}%</span>
                            @endif
                        </div>
                    </div>
                    <div class="flex justify-between mt-1 text-[10px]">
                        <span class="text-emerald-500 font-bold">🏠 Need Rp {{ number_format($needAmount, 0) }}</span>
                        <span class="text-pink-500 font-bold">🎉 Want Rp {{ number_format($wantAmount, 0) }}</span>
                    </div>
                </div>
                @endif
            </div>
            ...
            <div class="card lg:col-span-2">
                ...
                @php
                    ...
                    $mfCirc = 2 * 3.14159 * 70;
                    $mfInDash = $mfCirc * $inPct;
                    $mfOutDash = $mfCirc * (1 - $inPct);
                @endphp
                <div class="flex justify-center mb-4">
                    <div class="donut-wrap">
                        <svg width="180" height="180" viewBox="0 0 180 180">
                            <circle class="donut-track" cx="90" cy="90" r="70"/>
                            <circle class="donut-swing" cx="90" cy="90" r="70"
                                stroke="#f87171"
                                stroke-dasharray="{{ $mfOutDash }} {{ $mfCirc }}"
                                stroke-dashoffset="0"
                                transform="rotate(-90 90 90)"/>
                            <circle class="donut-swing" cx="90" cy="90" r="70"
                                stroke="#3b82f6"
                                stroke-dasharray="{{ $mfInDash }} {{ $mfCirc }}"
                                stroke-dashoffset="{{ $mfCirc * 0.25 }}"
                                transform="rotate(-90 90 90)"/>
                        </svg>
                        <div class="donut-center">
                            <span class="text-2xl font-black text-gray-900 count-anim">
                ...
                <div class="flex justify-center gap-6 text-xs font-semibold mb-3">
                ...
                <div class="grid grid-cols-3 gap-2 text-center pt-3 border-t border-gray-100">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-gray-400">Daily Avg</p>
                    </div>
                    <div>
                        <p class="text-xs font-bold text-gray-400 mb-0.5">{{ $transactionCount }}</p>
                        <p class="text-[10px] text-gray-400">Txns</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bar-wrap mt-4">
                @foreach($monthlyTrend as $m)
                    ...
                    <div class="bar-group" title="{{ $m['label'] }}: In Rp{{ number_format($m['in'],0) }}, Out Rp{{ number_format($m['out'],0) }}">
                        <div class="bar-stack">
                            <div class="bar-segment in" style="height: {{ $hIn }}px"
                                data-label="{{ $m['label'] }}"
                                data-in="{{ number_format($m['in'], 0) }}"
                                data-out="{{ number_format($m['out'], 0) }}"
                                data-net="{{ number_format($net, 0) }}"
                                data-type="in"></div>
                            <div class="bar-segment out" style="height: {{ $hOut }}px"
                                data-label="{{ $m['label'] }}"
                                data-in="{{ number_format($m['in'], 0) }}"
                                data-out="{{ number_format($m['out'], 0) }}"
                                data-net="{{ number_format($net, 0) }}"
            ...
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h3 class="text-base font-bold text-gray-900 flex items-center gap-2">
                        <i class="ph ph-wallet text-indigo-500"></i>
                        Monthly Budgets
                    </h3>
                    <p class="text-xs text-gray-400 mt-0.5">This month's spending vs budgets</p>
                </div>
                <a href="/expenses" class="text-xs font-bold text-indigo-500 hover:text-indigo-700 px-3 py-1.5 rounded-xl hover:bg-indigo-50 transition border border-indigo-200">
        ...
        <!-- Savings Goal -->
        @if(!empty($savingsProgress))
        <div class="card">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h3 class="text-base font-bold text-gray-900 flex items-center gap-2">
                        <i class="ph ph-target text-pink-500"></i>
                        Monthly Savings Goal
                    </h3>
                    <p class="text-xs text-gray-400 mt-0.5">Rp {{ number_format($savingsGoal, 0) }} target</p>
                </div>
                <a href="/savings-goal" class="text-xs font-bold text-pink-500 hover:text-pink-700 px-3 py-1.5 rounded-xl hover:bg-pink-50 transition border border-pink-200">
                    <i class="ph ph-pencil-simple mr-0.5"></i>Edit Goal
                </a>
            </div>
            <div class="flex items-center gap-6">
                <div class="text-center">
                    <p class="text-3xl font-black text-pink-600">Rp {{ number_format($savingsProgress['saved'], 0) }}</p>
                    <p class="text-xs text-gray-400">saved so far</p>
                </div>
                <div class="flex-1">
                    <div class="w-full bg-gray-100 rounded-full h-4 overflow-hidden mb-2">
                        <div class="h-full rounded-full transition-all duration-700 {{ $savingsProgress['on_track'] ? 'bg-emerald-400' : 'bg-amber-400' }}"
                            style="width: {{ $savingsProgress['progress'] }}%"></div>
                    </div>
                    <div class="flex justify-between text-xs text-gray-400">
                        <span>{{ $savingsProgress['progress'] }}%</span>
                        <span>{{ $savingsProgress['remaining'] > 0 ? 'Rp ' . number_format($savingsProgress['remaining'], 0) . ' to go' : 'Goal reached!' }}</span>
                    </div>
                    <div class="flex gap-4 mt-2">
                        <div class="text-center">
                            <p class="text-xs font-bold text-gray-500">Expected by now</p>
                            <p class="text-xs font-black text-gray-700">Rp {{ number_format($savingsProgress['expected_by_now'], 0) }}</p>
                        </div>
                        <div class="text-center">
                            <p class="text-xs font-bold {{ $savingsProgress['on_track'] ? 'text-emerald-600' : 'text-amber-600' }}">Status</p>
                            <p class="text-xs font-black {{ $savingsProgress['on_track'] ? 'text-emerald-700' : 'text-amber-700' }}">{{ $savingsProgress['on_track'] ? 'On Track' : 'Behind Pace' }}</p>
                        </div>
                        <div class="text-center">
                            <p class="text-xs font-bold text-gray-500">Projected</p>
                            <p class="text-xs font-black text-gray-700">Rp {{ number_format($savingsProgress['projected'], 0) }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="card">
            <div class="flex items-center justify-between mb-4">
                ...
                <button onclick="openTemplateModal()"
                    ...
                    <i class="ph ph-{{ empty($distributionTemplate) ? 'plus' : 'pencil-simple' }} mr-1"></i>
                ...
                <div class="flex gap-2 mb-3">
                    @foreach($distributionTemplate as $i => $item)
                        <div class="flex-1 rounded-2xl p-3 text-center"
                            ...
                            <div class="text-[10px] text-gray-500 font-semibold truncate">{{ $item['category'] }}</div>
                        </div>
                    @endforeach
                </div>
                    ...
                    <div class="space-y-3 mt-4">
                        @foreach($distributionChart as $item)
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-1">
                                    <div class="flex items-center gap-2">
                                        <span class="text-base">{{ $templateIcons[collect($distributionTemplate)->search(fn($t) => $t['category'] === $item['category']) ?? 0] ?? '📦' }}</span>
                                ...
                                <div class="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
                                    <div class="h-full rounded-full transition-all duration-700"
                                        style="width: {{ $maxDist > 0 ? round($item['amount'] / $maxDist * 100) : 0 }}%; background-color: {{ $templateColors[collect($distributionTemplate)->search(fn($t) => $t['category'] === $item['category']) ?? 0] ?? '#6b7280' }};"></div>
                                </div>
                            </div>
                            @endif
                        @endforeach
        ...
        <div class="modal-box">
            ...
            <div id="distSection" class="hidden">
                <div class="border-t border-gray-100 pt-4 mb-4">
                    ...
                    <div id="distCategories" class="space-y-2"></div>

                    <div class="mt-3">
                        <div class="flex gap-1 h-3 rounded-full overflow-hidden" id="distBarPreview"></div>
    ...
    <div id="expenseModal" class="modal-overlay" onclick="if(event.target===this)closeExpenseModal()">
        <div class="modal-box">
            ...
            <div id="catReveal" class="cat-reveal">
                ...
                <div id="catChips" class="flex flex-wrap gap-2 justify-center"></div>
            </div>

            <!-- Distribution deduction selector -->
    ...
    <div id="templateModal" class="modal-overlay" onclick="if(event.target===this)closeTemplateModal()">
        <div class="modal-box" style="max-width:520px">
            ...
            <div id="tmplCategories" class="space-y-2 mb-3 max-h-60 overflow-y-auto"></div>
            ...
            <div class="mb-3">
                <div class="flex gap-1 h-4 rounded-full overflow-hidden" id="tmplPreviewBar"></div>
                <div class="flex justify-between mt-1">
    ...
    <script>
        ...
        const PRESETS = { balanced: 50, saver: 80, spender: 20 };
        ...
        function sortFlat(col, th) {
            ...
            rows.sort((a, b) => {
                ...
                return sortFlat.dir === 'asc' ? va.localeCompare(vb) : vb.localeCompare(va);
            });
        ...
        window.addEventListener('load', () => {
            ...
            if (gauge) {
                ...
                setTimeout(() => {
                    gauge.style.strokeDashoffset = total * (1 - {{ $healthScore }}/100);
                }, 100);
            }
        });
        ...
        let distState = { enabled: false, total: 0, amounts: {}, pct: {}, totalPct: 0 };
        ...
        function getDefaultCategories() {
            return [
                { category: 'Emergency Fund', color: '#22c55e', icon: '🛡️' },
                { category: 'Investment', color: '#3b82f6', icon: '📈' },
                { category: 'Goals', color: '#f59e0b', icon: '🎯' },
                { category: 'Buffer', color: '#a78bfa', icon: '💼' },
            ];
        }

        function getCategoriesFromTemplate() {
            if (DIST_TEMPLATE.length > 0) {
                return DIST_TEMPLATE.map((t, i) => ({
                    category: t.category,
                    color: t.color || DIST_COLORS[i] || '#6b7280',
                    icon: t.icon || DIST_ICONS[i] || '📦',
                    pct: t.pct || 0,
                }));
            }
        ...
        function openSplitModal(form) {
            if (!form.description.value.trim()) { alert('Please enter a description first!'); form.description.focus(); return; }
            if (!form.amount.value || parseFloat(form.amount.value) <= 0) { alert('Please enter an amount!'); form.amount.focus(); return; }
            pendingForm = form;
            splitAmount = parseFloat(form.amount.value);
            splitDesc = form.description.value.trim();
            splitDate = form.transaction_date.value;

            document.getElementById('splitAmountDisplay').textContent = 'Rp ' + splitAmount.toLocaleString('id-ID');
            document.getElementById('splitDescDisplay').textContent = 'from: ' + splitDesc;
        ...
        function openSplitModalForAmount(amount, desc) {
            ...
            document.getElementById('splitAmountDisplay').textContent = 'Rp ' + amount.toLocaleString('id-ID');
            document.getElementById('splitDescDisplay').textContent = 'from: ' + desc;
            const coin = document.getElementById('splitCoin');
            coin.classList.remove('coin-flip');
            void coin.offsetWidth;
            coin.classList.add('coin-flip');
        ...
        function updateSplit(savePct) {
            ...
            document.getElementById('amtSaveDisplay').textContent = 'Rp ' + Math.round(saveAmt).toLocaleString('id-ID');
            document.getElementById('amtSpendDisplay').textContent = 'Rp ' + Math.round(spendAmt).toLocaleString('id-ID');
            ...
            const msgs = { 80: "Maximum saver mode! You're a legend! 🚀", 70: "Whoa, you're on fire! 🔥", 60: 'Solid saving discipline! Keep it up! 💪', 50: 'A great split starts your month right! 🎯', 30: 'Treat yourself, but wisely! ✨', 0: 'Every split counts — great job! 🎯' };
            let msg = msgs[0];
            for (const [threshold, text] of Object.entries(msgs)) { if (pct >= parseInt(threshold)) msg = text; }
            document.getElementById('splitMessage').textContent = msg;

            if (pct > 0 && !distState.enabled) openDistSection();
            else if (pct === 0) closeDistSection();
        }
        ...
        function openDistSection() {
            ...
            distState.amounts = {};
            distState.pct = {};
            distState.totalPct = 0;

            const cats = DIST_TEMPLATE.length > 0 ? DIST_TEMPLATE : getDefaultCategories();
            cats.forEach(cat => {
                const pct = cat.pct || (100 / cats.length);
                distState.amounts[cat.category] = saveAmt * pct / 100;
                distState.pct[cat.category] = pct;
            });
            distState.totalPct = cats.reduce((sum, c) => sum + (c.pct || (100 / cats.length)), 0);
        ...
        function buildDistCategoryUI() {
            ...
            cats.forEach((cat, idx) => {
                const pct = distState.pct[cat.category] ?? 0;
                const amt = distState.amounts[cat.category] ?? 0;
                ...
                container.innerHTML += `
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs font-semibold text-gray-600 w-24 truncate">${cat.category}</span>
                        <input type="range" min="0" max="100" value="${pct}"
                            class="flex-1 h-1.5 rounded-full cursor-pointer"
                            style="accent-color:${color}"
                            oninput="updateDistPct('${cat.category.replace(/'/g, "\\'")}', this.value, '${color}')">
        ...
        function submitWithSplit() {
            ...
            if (distState.enabled && distState.totalPct === 100) {
                const dist = cats.map((cat, idx) => ({
                    ...
                    pct: distState.pct[cat.category] || (DIST_TEMPLATE[idx] ? DIST_TEMPLATE[idx].pct : 0),
                    icon: DIST_TEMPLATE[idx] ? DIST_TEMPLATE[idx].icon : '📦',
                    amount: Math.round(distState.amounts[cat.category] || 0),
                })).filter(d => d.pct > 0);
            ...
            closeSplitModal();
            triggerConfetti();
            showToast(`Rp ${Math.round(saveAmt).toLocaleString('id-ID')} saved! Great job! 🎉`);
        ...
        function openTemplateModal() {
            if (DIST_TEMPLATE.length > 0) {
                tmplState.rows = DIST_TEMPLATE.map(c => ({ name: c.category, pct: c.pct, icon: c.icon || '📦', color: c.color || '' }));
            } else {
                tmplState.rows = [
                    { name: 'Emergency Fund', pct: 50, icon: '🛡️', color: '#22c55e' },
                    { name: 'Investment', pct: 50, icon: '📈', color: '#3b82f6' },
                ];
            }
            renderTemplateRows();
        ...
        function addTemplateRow() {
            tmplState.rows.push({ name: '', pct: 0, icon: '📦', color: '' });
            renderTemplateRows();
        }
        ...
        const COLOR_OPTIONS = ['#22c55e', '#3b82f6', '#f59e0b', '#a78bfa', '#f472b6', '#fb923c', '#14b8a6', '#e879f9'];

        function renderTemplateRows() {
            const container = document.getElementById('tmplCategories');
            container.innerHTML = '';
            tmplState.rows.forEach((row, idx) => {
                const iconOptions = ICON_OPTIONS.map(ic => `<option value="${ic}" ${row.icon === ic ? 'selected' : ''}>${ic}</option>`).join('');
                const colorOptions = COLOR_OPTIONS.map((c, i) => `<option value="${c}" ${row.color === c ? 'selected' : ''}>${c}</option>`).join('');
                container.innerHTML += `
                    <div class="flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-2">
                        ...
                        <select class="border border-gray-200 rounded-lg px-1 py-1 text-xs w-10 text-center bg-white"
                            onchange="tmplState.rows[${idx}].color = this.value; renderTemplateRows()">
                            ${colorOptions}
                        </select>
        ...
        function updateTemplatePreview() {
            ...
            tmplState.rows.forEach((row, i) => {
                ...
                if (pct > 0) {
                    const color = row.color || DIST_COLORS[i] || '#6b7280';
                    bar.innerHTML += `<div style="width:${pct}%;background:${color}" class="h-full transition-all duration-200 rounded-full"></div>`;
                }
            });
        }
        ...
        function saveTemplate() {
            const total = tmplState.rows.reduce((sum, r) => sum + (parseInt(r.pct) || 0), 0);
            if (total !== 100) { alert('Total must equal 100%!'); return; }
            const savePct = document.getElementById('tmplSaveSlider').value;
            // Normalize to 'category' key
            const normalized = tmplState.rows.map(r => ({
                category: r.name,
                pct: parseInt(r.pct) || 0,
                icon: r.icon || '📦',
                color: r.color || '',
            }));
            document.getElementById('distTemplateData').value = JSON.stringify(normalized);
        ...
        function openExpenseModalForAmount(amount, desc) {
            ...
            document.getElementById('expenseAmountDisplay').textContent = 'Rp ' + amount.toLocaleString('id-ID');
            document.getElementById('expenseDescDisplay').textContent = 'for: ' + desc;
            selectedNow = null;
            selectedCat = null;
            document.getElementById('nwBtnNeed').classList.remove('active');
            document.getElementById('nwBtnWant').classList.remove('active');
        ...
        function submitWithCategory() {
            ...
            closeExpenseModal();
            const msg = selectedNow === 'need' ? '🏠 Tagged as Need! Smart move.' : selectedNow === 'want' ? '🎉 Tagged as Want! Enjoy wisely.' : 'Expense logged!';
        ...
        // Strip distribution URL param if present
        (function() {
